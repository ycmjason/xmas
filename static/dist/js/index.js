...
var removeClass = function removeClass(elem, cls) {
  return elem.className = elem.className.split(/\s+/g).filter(function (c) {
    return c !== cls;
  }).join(' ');
};

var addClass = function addClass(elem) {
  for (var _len = arguments.length, clses = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    clses[_key - 1] = arguments[_key];
  }

  return elem.className = [].concat(_toConsumableArray(elem.className.split(/\s+/g)), clses).join(' ');
};
